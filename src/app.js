    ...
    const app = {
        ...
        screens: {
            'main_menu': function () {
                ...
                appBlock.querySelector('.main_menu-levels').addEventListener('click', function (e) {
                    if (e.target.tagName === 'INPUT') {
                        e.stopPropagation();

                        const levels = appBlock.querySelectorAll('.main_menu-levels-item input');
                ...
                appBlock.querySelector('form').addEventListener('submit', function (e) {
                    e.preventDefault();
            ...
            game: function () {
                ...
                const gameContentElement = appBlock.appendChild(templateEngine({
                    tag: 'div',
                    cls: ['game-content', `game-content-grid-${gameDifficultyLevel}`]
                }));
                ...
                gameContentElement.addEventListener('click', (e) => {
                    ...
                    if (target.classList.contains('card-front')) {
                        target.parentElement.classList.add('game-content-card-show');

                        //animation triggers one time so it's necessary to remove class with animation
                        setTimeout(() => { target.parentElement.classList.remove('game-content-card-show') }, 5000)
                    }
                });
        ...
        renderScreen: function (screenName) {
            ...
            appBlock.classList.add(screenName);
            appBlock.dataset.specialClass = screenName;
